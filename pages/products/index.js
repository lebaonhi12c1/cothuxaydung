  ...
  const filterProducts = (inputProducts) => {
    ...
    if (inputProducts && inputProducts.length > 0) {
      filteredProducts = inputProducts.filter((product) => {
        if (category === "all" || product.categoryid.name === category) {
          if (price === "") {
            return true;
          } else if (price === "0-100") {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {filteredProducts.length > 0 ? (
          filteredProducts.map((product) => (
            <div
              key={product.name}
              className="border border-gray-400 rounded-lg shadow-sm p-4 hover:shadow-2xl transition duration-150 overflow-hidden"
            >
...
export const getStaticProps = async ()=>{
  ...
  const res_category = await fetch(`${process.env.SERVER_URL}/category`)
